...
def Findlines (binary_warped):
    ...
    margin = 80 # margin of search window
    ...
    if (left_fit!= [] and right_fit!=[]): # as they are global variables I am checking if this is the first frame or there is an existing history  
        ...
        if(left_fit[0]-right_fit[0])>1e-4 and (left_fit[1]-right_fit[1])>1e-1: # in case left and right lines coefficient are far so the lines are not parallel and we need a search window 
                left_lane_inds = []
                right_lane_inds = []
